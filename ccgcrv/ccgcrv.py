...
from gdess.formatters.args import options_to_args
...
def _main(options):
	startdate = validate_options(options)
	...
	if not options.user and not options.equal:
		options.sample = True
	# Compute the filtered data
	if options.timez is None:
		options.timez = int(xp[0])
	filt = ccgFilter(xp, yp, options.short, options.long, options.interv, options.npoly, options.nharm,
								options.timez, options.gap, options.gain)
...
def _ccgcrv_parser():
	...
	group.add_argument('--gap', default=0, type=int,
					   help="Fill gaps larger than GAP days with function value instead of linear interpolation.")
	group.add_argument('--gain', action="store_true", default=False,
					   help="Use seasonal amplitude gain factor in function fit.")
	group.add_argument('--timez', type=float,
					   help="Specify time zero for coefficients of function. Default is year of first data point.")

	group = parser.add_argument_group("Output Options")
	group.add_argument('-f', '--file',
					   help="Write equally spaced or user spaced output data to file instead of stdout.")
	...
	group.add_argument('--sample', action="store_true", default=False,
					   help="Output data at sample data times (default).")
	...
	group.add_argument('--showheader', action="store_true", default=False,
					   help="Include header on output to identify columns.")
	...
	group.add_argument('--smres', action="store_true", default=False,
					   help="Output smoothed residuals from the function.")
	group.add_argument('--trres', action="store_true", default=False,
					   help="Output long-term smoothed residuals from the function.")
	...
	group.add_argument('--stats', action="store_true", default=False,
					   help="Output table of summary statistics for curve fit.")
	group.add_argument('--amp', action="store_true", default=False,
					   help="Output table of statistics for annual amplitudes.")
	group.add_argument('--mm', action="store_true", default=False,
					   help="Output Monthly means computed using smooth curve.")
	group.add_argument('--annual', action="store_true", default=False,
					   help="Output Annual means computed using smooth curve.")

	return parser


if __name__ == "__main__":
	options = _ccgcrv_parser().add_argument('args', nargs=1).parse_args()
	_main(options)


def ccgcrv(options: dict, data_file):
